...
<template>
  ...
  <div
    ...
    :key="index"
    class="hidden lg:flex w-full max-w-5xl justify-center items-center mx-auto rounded-md flex-col lg:flex-row gap-3"
  >
    <div
      class="h-full overflow-hidden rounded-t-md lg:max-w-md shadow-xl"
      :class="{ 'order-2': index % 2 !== 0 }"
    >
      <img
        :src="`/images/${about.image}`"
        :alt="`Image Preview of ${about.name}`"
        class="aspect-video rounded-t-md object-cover lg:rounded-md"
      />
    </div>
    <div>
      <p
